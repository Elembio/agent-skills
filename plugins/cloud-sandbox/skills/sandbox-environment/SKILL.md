...
5. **Return results** — write figures/tables to `$ELEMBIO_EXECUTION_OUTPUTS_DIR`, then
   `fetch_artifact` the path and paste an image's `display_markdown` verbatim.
...
- The handling rules (revise-by-new-file, the full `s3_status` values, `fetch_artifact`'s
